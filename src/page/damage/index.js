...
class Damage extends Component{
    ...
    InsertData = (value) => {
        axios.post('http://localhost:8089/dormitory/damsave',value)
            .then(response => {
    ...
    SearchData = () => {
        ...
        axios.post('http://localhost:8089/dormitory/damsearchlist',this.state.search)
            .then(response => {
    ...
    DeleteData = () => {
        ...
        axios.post('http://localhost:8089/dormitory/damdelete', this.state.selectedRows)
            .then(response => {
    ...
    GetData = () => {
        axios.defaults.headers.common['Authorization'] = sessionStorage.getItem("token")
        axios.get('http://localhost:8089/dormitory/damlist')
            .then(response => {
                console.log("response: ", response);
                if (response.data.state) {
                    this.setState({list: response.data.datalist});
                ...
                <div>
                    ...
                    >
                        <Form {...layout} name="nest-messages" ref={this.formRef} validateMessages={validateMessages} onFinish={this.InsertData}>
                            <Form.Item name="da_id" label="da_id" initialValue={this.state.uplist.da_id} hidden={true}>
                            </Form.Item>
                            <Form.Item name={['damagestation']} label="损耗物品" initialValue={this.state.uplist.damagestation} rules={[{required: true}]}>
                                <Input/>
                            </Form.Item>
                            <Form.Item name={['damagetime']} label="损耗时间"
                                       initialValue={this.state.selectedRows.length == 0 ? "" : moment(this.state.uplist.damagetime, 'YYYY-MM-DD')}>
                                <DatePicker/>
                            </Form.Item>
                            <Form.Item name={['isfixed']} label="是否修复" initialValue={this.state.uplist.isfixed}>
                                <Input/>
                            </Form.Item>
