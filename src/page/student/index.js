...
class Student extends Component{
    ...
    InsertData = (value) => {
        axios.post('http://localhost:8089/dormitory/stusave',value)
            .then(response => {
    ...
    SearchData = () => {
        ...
        axios.post('http://localhost:8089/dormitory/stusearchlist',this.state.search)
            .then(response => {
    ...
    GetData = () => {
        axios.defaults.headers.common['Authorization'] = sessionStorage.getItem("token")
        axios.get('http://localhost:8089/dormitory/studentlist')
            .then(response => {
                console.log("response: ", response);
                if (response.data.state) {
                    this.setState({list: response.data.datalist});
    ...
    DeleteData = () => {
        ...
        axios.post('http://localhost:8089/dormitory/studelete', this.state.selectedRows)
            .then(response => {
                if (response.data.state) {
                ...
                <div>
                    ...
                    >
                        <Form {...layout} name="nest-messages" ref={this.formRef}
                              validateMessages={validateMessages} onFinish={this.InsertData}>
                            <Form.Item name={['S_id']} label="学号" initialValue={this.state.uplist.s_id} rules={[{required: true}]}>
                                <Input/>
                            </Form.Item>
                            <Form.Item name={['S_name']} initialValue={this.state.uplist.s_name} label="姓名">
                                <Input/>
                            </Form.Item>
                            <Form.Item name={['sex']} initialValue={this.state.uplist.sex} label="性别">
                                <Input/>
                            </Form.Item>
                            <Form.Item name={['birthday']} label="生日"
                                       initialValue={this.state.selectedRows.length == 0 ? "" : moment(this.state.uplist.birthday, 'YYYY-MM-DD')}>
                                <DatePicker/>
                            </Form.Item>
                            <Form.Item name={['qq']} label="qq" initialValue={this.state.uplist.qq}>
                                <Input/>
                            </Form.Item>
                            <Form.Item name={['mobile']} label="手机号" initialValue={this.state.uplist.mobile}>
                                <Input/>
                            </Form.Item>
